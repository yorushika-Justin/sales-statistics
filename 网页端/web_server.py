...
from 汇总脚本 import read_xls, read_xlsx, draw_table, read_xls_items, read_xlsx_items, draw_table_detail, OUTPUT_DIR

# 端口常量（open_browser 和 app.run 共用，改端口只需改此处）
PORT = 5000
...
def open_browser():
    ...
    v1.1.1 修复：用 os.startfile 替代 webbrowser.open，
    解决 pythonw + webbrowser.open 静默启动下不开浏览器的问题。
    v1.2 修复：用 socket 轮询探测端口就绪，替代 time.sleep(1) 硬等待。
        ...
        if 'address already in use' in str(e) or '通常每个套接字地址' in str(e):
            if not silent_mode:
                _show_error_popup(
                    ...
                    f'2. 或修改 web_server.py 顶部 PORT 常量（当前为 {PORT}）'
                )
        else:
            if not silent_mode:
